...
\section{Something Different: A Role For Contrast}
...
Each model was used to simulate each participant's generation independently, with the importance of each dimension set according to the participant's generated category. The other free parameters within each model were set as in Table \ref{table:global-model-fits}. Every participant's generation was simulated 2,000 times; multiplied against 305 total participants, each model generated 610,000 categories in total. For comparison with our behavioral results, we then computed the range difference gradient identically as with the behavioral data. The results are shown in Figure \ref{fig:range-diff-gradients}.
...
The creative generation of concepts is an greatly intriguing topic, but it is also of great complexity and can be difficult to study in a traditional laboratory setting. Creative generation is most typically studied using a creative cognition approach \citep{finke1992creative,smith1995creative}, where creative products \citep[such as drawings of alien plants and animals;][]{ward1994structured} are analyzed to obtain insight into the processes and representations involved. While this approach has yielded a great deal of information about the nature of creative generation, the commonly employed response modalities (e.g., hand-drawn pictures) made difficult the development of computational models to formally evaluate the proposed representations and processes. However, the generation of novel concepts is not altogether different from the types of behaviors studied in cognitive psychology: In particular, it can be viewed as an form of inference using one's knowledge about existing categories \citep{kemp2014taxonomy,kurtz2015human}. Taking advantage of this insight, in this paper we investigated the generation of new categories using the empirical and computational toolkit from the field of categorization.
