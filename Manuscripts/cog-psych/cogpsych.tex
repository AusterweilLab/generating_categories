...
In Experiment 1 we sought to test whether category contrast systematically influences categories that are creatively generated by participants. We found strong evidence for effects of category contrast: participants were more likely to generate stimuli that are more distant from (i.e., less similar to) members of a previously-learned category, and members of participant-generated categories tended to be more similar to one another than to members of previously-learned categories. We also partially replicated the classic finding that the distributional structure of generated categories reflects that of previously learned categories \citep{jern2013probabilistic,ward1994structured}: members of generated categories were more widely distributed along dimensions which were widely distributed in the experimenter-defined category. 

A notable divergence, however, was the finding that the feature correlations within the categories generated in the XOR condition tended oppose that of the experimenter-defined category. While this is difficult to account for within existing theoretical approaches \citep[i.e.,][]{jern2013probabilistic}, it can be concisely explained from a category contrast perspective. Within the XOR condition, individuals who seek to generate a category that is perceptually distinct from what is already known are left with only the upper-left and bottom-right quadrants, as members of the experimenter-defined XOR category lie in the bottom-right and top-left quadrants. If examples are generated into both of the available quadrants, the generated category will possess a strongly negative correlation, opposing that of the experimenter-defined class.

The negative correlations observed in the XOR condition may therefore indicate something further about the role of category contrast: constraints on creative generation imposed by category contrast may not simply influence the \textit{location} of generated categories, but also their distributional structure. In Experiment 2, we test this claim more systematically.


\section{Experiment 2}

In Experiment 1, we observed that participants are less likely to generate stimuli that are highly similar to members of existing categories. Effectively, this demonstrates that members of known categories influence where new categories will be generated within the domain. A related question, however, concerns whether category contrast influences the distributional structure of generated categories. Specifically, does the shape of the unoccupied area within the domain influence the types of categories people generate?
...
To address this question, we developed two new categories within the same domain as Experiment 1 (see Figure \ref{fig:e2-conditions}). The categories possess an identical distributional (both are tight clusters of examples with a neutral feature correlation), and only differ slightly in their Y-axis position: the `Bottom' category is a tight cluster in the bottom-center of the space, and the `Middle' category lies in the center of stimulus space. 

By consequence, approaches focusing on the emulation of distributional structure \citep[i.e.,][]{jern2013probabilistic} would fail to predict any differences in the types of categories generated after learning the 

Importantly, however, the slight change to the position along the Y-axis produces considerable changes to the shape of the space that remains unoccupied after learning. 
