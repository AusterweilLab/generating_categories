\message{ !name(cogpsych.tex)}%!TEX output_directory = latex_out/
...
\message{ !name(cogpsych.tex) !offset(-3) }
...
\sxlnote{Updated. I tried writing an outline, but found that points got a little
  repetitive after a bit, so I ditched that idea... Might have just been the way
I wrote it though.}
...
\jlanote{Xian: Please do a Chi-Squared test between what is expected based on
  the logic above and the observed frequencies mentioned below}
\sxlnote{I didn't do a chi-sq test because what appears below is not the
  observed frequencies that correspond to the logic above, but is instead just
  an indication of whether or not it is reasonable to assume the logic above
  (and the argument is that it is not reasonable.)}
...
\message{ !name(cogpsych.tex) !offset(-2294) }
