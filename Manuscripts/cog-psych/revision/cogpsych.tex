...
representative of the domain (and thus, not new at all).

\sxlnote{I think we can address Charles' point on real world examples of how
  contrast can be important around here.}
...
\jlanote{check to make sure the next paragraph has been updated enough}
\sxlnote{I think it's updated enough! If given a chance I might edit this a
  little to be less harsh on hierarchical bayesian models in general though I
  think, since the representativeness model is one of them?}
...
models. In fact, our model of contrast as representativeness is a hierarchical
Bayesian model, but with a different goal than predicting the next category.
...
\sxlnote{Charles suggested we either remove the section below, or move it to the
  discussion. I've tried finding a good place for it in the discussion but it
  doesn't fit very well, and moving it takes away this nice motivation for
  our further investigation into contrast. I think that keeping it here in the
  introduction might work out, provided Charles finds my argument for why the
  regularity assumption is too strong convincing enough. More details a little
  further down.}
...
\sxlnote{Added this bit on regularity below - I'm a little undecided as to leave
  it in the main text or place as a gigantic footnote? It is a bit of a
  digression, and it's really just here to address Charles' point.... If we want
  to keep this in the main text, I'll have to amend the following paragraph
  ``Re-analyzing the results...'' to improve the transition.}
%
...
However, upon analysing the consistency of hues in the generated categories, it
...
random. Crucially, however, the next generated exemplar from the
...
MAKE SURE WE CITE AND DESCRIBE \citet{thomas98}! \sxlnote{I moved the
  description and citation of thomas98 to the introduction for justification of
  why we think categories should share similar distributions.}
