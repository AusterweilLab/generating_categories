...
\subsection{Disentangling Exemplar dissimilarity and Representativeness in Categorization} 
...
In this paper, we have developed two novel models of category generation, each
involving a different conceptualization of contrast.  However, there is no theoretical constraint that dictates the It is also possible to treat the similarity

A thorough investigation to disentangle the two contrast mechanisms here is
beyond the scope of the current article. However, as a quick proof of concept it
is useful to check if the addition of a representativeness mechanism to an
exemplar model would yield any benefit at all. 
