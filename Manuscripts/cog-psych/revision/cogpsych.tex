...
Much of what we know about concept generation and contrast comes from the foundational
...
\sxlnote{Added thomas98 description here} \jlanote{still need the contrast result from her work.} It is worth pointing out that the
observation of regularities in distributional properties across categories is
not confined to generating whole new categories, but it also occures when inferring an unobserved feature within traditional categorization paradigms. Most notably, \citet{thomas98}
...
\jlanote{I'm a bit confused on the theta constraints. it's clear they need to be non-negative, but you don't usually see both needing to be between 0 and 1 but not sum to some value. is this really what we ended up with? Something feels fishy to me with the math.}
...
$f(x_j) = -\theta_c$ if $x_j$ is a member of a contrast category.
$\theta_t$ and $\theta_c$ are free parameters
($0 \leq (\theta_t,\theta_c) \leq 1$) controlling the trade-off
between within- and between-category similarity. For example, when
$\theta_t = \theta_c = 0.5$, $f(x_j) = 0.5$ for members of the
target category and $f(x_j) = -0.5$ for members of other categories; thus, the
model is likely to generate items that are similar to members of the target
category but are not similar to members of other categories. In this way,
$\theta_t > 0$ with $\theta_c = 0$ produces exclusive consideration
of target-category members, and $\theta_c > 0$ with $\theta_t = 0$
produces exclusive consideration of contrast-category members. The combination
of $\theta_t$ and $\theta_c$ parameters thus specifies a wide
breadth of possible approaches; by fitting it to a dataset, one can describe the
relative roles of between-category contrast and within-category similarity in
generation. See Figure \ref{fig:packer-examples} for an illustration of how
these parameters control the relative influence of within-category similarity
and contrast to other categories when generating a new exemplar.
...
representative of the underlying distribution for a target category. \jlanote{The next sentence is weird sounding and should be cleaned up} Generally,
this representativeness mechanism results in the generation of exemplars that
are less similar (i.e., more different) to contrast categories. We see this
illustrated in Figure \ref{fig:representative-examples}, where the
...
\sxlnote{For this section I've added the existence of the representativeness
  model but I think discussion of its performance is still lacking compared to PACKER.}
Experiments 1 and 2 revealed systematic and strong effects of category contrast
on creative generation. In this section, we analyze the performance of the different formal models at explaining the experimental results. Specifically, we
...
\sxlnote{The argument in the following two paragraphs appear to be challenged by
  the success of the representativeness model. I might suggest removing them altogether?}
...
A key distinction between PACKER and the other three models is that only PACKER
is capable of making strong predictions about the location of new category
members when the target class is entirely novel (i.e., no member of the category
has been observed). Under these circumstances, there are no examples to copy,
and thus the copy-and-tweak model predicts that items are generated at random.
Likewise, with no observations on which to condition the category distribution,

\jlanote{Do they really pick the first item at random? That doesn't seem right for the representativeness -- shouldn't it be constrained?}
the hierarchical Bayesian and representativeness models also picks an item at
random. 

Crucially, however, the next generated exemplar from the
representativeness model is strongly influenced by contrast as
representativeness. Thus, it is possible that the failure of these models is
simply due to their inability to explain each participant's first trial
(generating the first item in the `Beta' category). To ensure this is not
driving our results, we conducted an identical set of simulations as above,
excluding this trial (leaving 915 responses in the dataset): Again, PACKER
($L = -3390$, $AIC = 6786$) achieved better fits than the copy-and-tweak
($L = -3579$, $AIC = 7162$) and hierarchical Bayesian ($L=-3612$, $AIC = 7232$)
models.
\jlanote{last sentence missing representativeness fit}
...
\jlanote{Xian: Please add a paragraph or two on the representativeness model here}
...
We conducted simulations comparing contrast accounts of our results to the other
major proposals for category generation from prior work: a ``copy-and-tweak'' model (realized as
...
similarity effect. In all simulations, we found that 
contrast captured a previously unexplained and unexplored aspect of human
category generation. In the case of contrast as exemplar dissimilarity, we operationalized contrast by measuring PACKER's fit as a function of its
...
\jlanote{Above needs some updating still. Also, do we want an outline of general discussion paragraph?}

\subsection{Contrast Effects in Prior Work}
...
exemplars that all share a common hue. If this is the case, given a $6/8 = .75$
probability that a hue distinct from the experimenter-defined categories is
selected, there is a corresponding 75\%  chance that the generated category will have no exemplars that share a
hue with any of the experimenter-defined categories. Conversely, that there is
a 25\% chance that the generated
...
\jlanote{Xian: Please do a Chi-Squared test between what is expected based on the logic above and the observed frequencies mentioned below.}
...
\jlanote{this needs updating for representativeness.}
...
creative cognition, the PACKER model more specifically offers an additional
...
knowledge. The results of our simulations with PACKER demonstrate that this
influence is concisely explained in terms of a trade-off between within-category
...
\sxlnote{I'm struggling a little with this section! That said, I think this may
  also be a good point to address Charles' concern re: the possible conditions
  under which contrast are important.} 
\jlanote{Ken, could you take a stab at this section please?}
Although both PACKER and the
representativeness model perform better than the other models, they appear to be
capturing fundamentally different aspects of contrast in categorization.
Specifically, while the representativeness model excelled in predicting our
category generation data, PACKER was the best-fitting model in accounting for
participant error in category learning. The source of this discrepancy is still
not yet clear -- it could be an indication of a fundamental difference between
the processes involved in generation versus learning where exemplar
dissimilarity is a larger factor in category generation while representativeness
exerts more influence in category learning.

%Xian: Please
%write about the two hypotheses and that followup work needs to look into why the
%Bayesian representativeness model doesn't do as well at explaining category
%learning ease, but does better at category generation.
