...
similar to known category exemplars) when the known category distribution is
especially large on all features -- we explore this in more detail in Section
...
their fundamentally different structures, both contrast models seem to make
qualitatively similar predictions. To illustrate this, Figure
\ref{fig:firstbetas} shows how the modal generating probabilities for the first
Beta exemplar are similar across both contrast models.\footnote{Model parameters
  are set to the optimized values from Table \ref{table:global-model-fits}.} The
highest density regions for all models appear to be near the corners and edges.
This is particularly the case for the representativeness model, which almost
exclusively predicts generation at the corners, while PACKER can generate the
first novel exemplars toward the inner regions of the stimulus space in the XOR
condition.
...
unlike PACKER, the representativeness model does not explicitly use a
distance-based measure in determining exemplar generation probabilities. The
general pattern of producing exemplars dissimilar from the contrast category is
the result of the underlying unimodal category structure assumed by the model
and is not strictly constrained by the distances between exemplars.
Consequently, it is possible to design a configuration of Alpha exemplars where
...
In this section, we focus on one particularly useful category type -- the Corner
category -- to distinguish between the two competing accounts of contrast in
category generation. Here, each Alpha exemplar is located at the extreme corners
of the feature space (see Figure \ref{fig:e3-squares}a). We first seek to
identify what predictions each contrast model generates under this condition,
followed by an category generation task, and a brief modeling exercise to
determine which model is a better predictor of the behavioral data.

In the Corner condition, the representativeness model assumes an underlying
Alpha category distribution that is centred on the feature space, with
probability densities highest in the centre and lowest in the corners. Since the
representativeness of novel Beta exemplars generally increase when the
...
[Transition to and justification for this still feels a little shaky]

A large part of the representativeness model's predictive accuracy in
Experiments 1 and 2 is due to its preference for generating categories located
at the boundaries of the stimulus features. If at least one of these boundaries
is removed by utilizing an unbounded feature (i.e., no absolute maximum or
minimum exists in the stimulus space along this dimension), we should expect to
see a decrease in the predictive performance of the representativeness model. To
implement this, in our next experiment we also include a new set of stimuli:
...
details of this experiment, we will briefly explore the contrast model
...
y-axis. Due to the unbounded nature of orientation, the maximum distance from
the Alpha exemplars is half of the full range of the feature -- for an Alpha
exemplar oriented at 0 radians, the furthest another exemplar can be situated is
$\pi$ radians within the full range of orientation is $2\pi$ radians. 

For the representativeness model, the single unbounded feature essentially
prevents the formation of edge effects along that feature. Consequently, this
results in higher probabilities of the generating the first Beta exemplars along
the edges of the bounded feature (Figure \ref{fig:e3-circles}c). Since this
...
representativeness model (which prefers generating exemplars at the edges) will
be poorer at generating stimuli with an unbounded feature compared to
stimuli with all features bounded.
...
We recruited 89 participants from Amazon Mechanical Turk for this experiment, 46
of whom were randomly assigned square stimuli and the remaining 43 assigned
circle stimuli. Square stimuli were exactly the same as in Experiments 1 and 2.
Circle stimuli were made up of unfilled circles with a radial line extending
from its centre to the edge. These stimuli varied along two features -- their
diameter (or size) ranging from 3.0--5.8 cm, and their orientation ranging from
0--$2\pi$ radians. The orientation of each circle stimuli was fixed at a
randomly selected angle for all participants. Similar to Experiments 1 and 2,
participants first observed a training phase where four Alpha exemplars were
presented across three blocks followed by the generation phase. All participants
in this experiment were shown exemplars from the Corner category type.
...
Overall, we found no significant differences between the distributional statistics of each
generated category between square and circle stimuli (Figure
\ref{fig:e3-statsboxes}. The only exception is seen in the Y range (representing
the orientation of circle stimuli,) which is significantly lower for the circle
stimuli compared to the square stimuli ($t(87) = 4.94$, $p = .001$). This
...
At a rough level (see Table \ref{table:corner_counts}, 35\% of participants who
observed square stimuli tended to generate their first Beta exemplars along the
edges of the feature space. In comparison, only 12\% of participants who
observed circle stimuli generated their first Beta exemplars along the edges of
...
representativeness model (53\% of the former, 49\% probability of the latter).
In addition, these results agree with our prediction that the representativeness
model would be poorer in predicting data from circle stimuli (26\% of
participants observing circle stimuli) compared to square stimuli (39\%
participants observing square stimuli). [No proper stats here... feels a little
dirty. But I suppose there are model fits coming up.]

Quantitative model fits were also obtained by optimizing both contrast models to
the entire dataset including data from Experiments 1 and 2. Fit statistics and
parameter values are presented in Table \ref{table:corner-model-fits}. Overall,
the representativeness model still emerges as the best-fitting model, although
by a much smaller lead compared to fits to only Experiments 1 and
2. At an individual level, 67\% of participants in Experiment 3 were better
fit by PACKER than the representativeness model. 
...
\begin{tabular}{ l l l l} \\ Stimulus Type & \textbf{PACKER} &
                                                               \textbf{Representativeness}& Total \\
  \hline 
  Squares & 28 & 18 & 46\\
  Circles & 32 & 11 & 43\\
  Total   & 60 & 29 & 89\\
...
\begin{tabular}{ l l} \\ \textbf{PACKER} & \textbf{Representativeness} \\
  \hline $AIC = 12052$ & $AIC = 11991$ \\ 
  $L = -6023$          & $L = -5991$ \\ 
  $c = 0.60$           & $\kappa = 5.91$  \\ 
  $\theta_c = 2.78$    & $\nu = 1.00$ \\
  $\theta_t = 3.17$    & $\lambda = 1.45$ \\
                       &$\theta = 1.86$ \\
\end{tabular}
\end{table}

Ultimately, these results suggest that PACKER's exemplar dissimilarity account
of contrast, as opposed to the representativeness account, is more appropriate
for the Corner category type. On almost every measure of analysis in this
section, PACKER outperformed the representativeness model. Although the
representativeness model best fit the data from Experiments 1 and 2, it
struggled when the learned category occupied the extreme corners of the stimulus
space -- both in cases where the stimuli possess bounded and unbounded features.

[Work on transition time permitting]
...
As we explore in a later section, it is likely that the
representativeness mechanism itself (i.e., independent of the hierarchical
Bayesian foundation) captures a core aspect of both category generation and
category learning.
...
need to generate categories with entirely new exemplars. Additionally, the
...
excelled in predicting our overall category generation data, PACKER was the
best-fitting model in accounting for categories generated from the Corner
category type, where exemplars are located at the corners of the stimulus
domain. 
...
contrast mechanisms in category generation is still not entirely clear. Our
results could be an indication of a fundamental difference between the processes
involved where exemplar dissimilarity is a larger factor under certain category
configurations while representativeness exerts more influence in others.
Alternatively, it is possible they are both approximating some factor that we
still have not fully identified. While a thorough investigation is beyond the
scope of the current article, the preliminary results of this section indicate
that further work to disentangle these two mechanisms would be promising. In the
following section, we explore some other limitations of the current work and
provide more recommendations for future research.
