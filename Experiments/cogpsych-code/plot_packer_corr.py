...
catassign_corrs = "pickles/catassign_packer_corr2.p"
...
try:
    ...
    if parallelOn:
        ...
        def gcpar(parm):
            ...
            print '.'
            corr = gc.get_corr(parm,pptlist,Packer,info,assignment,stimuli,stats,WT_THETA,print_on = False)[0] #-trials.loglike(curr, Packer)
            return corr
        ...
        for i, val in enumerate(gamma_grid):
            ...
            corrs[i] = gc.get_corr(curr,pptlist,Packer,info,assignment,stimuli,stats,WT_THETA)[0] #-trials.loglike(curr, Packer)
            print corrs[i]
...
plt.text(1.2, copytweak_corrs-.08, 'Target Only (Copy & Tweak)', ha = 'center', va = 'bottom')
...
plt.savefig('packer-corr-t2.pdf', bbox_inches='tight', transparent=False)
